...
def evaluate_performance_of_simulation_model(I_S_0, sim, rep_num, print_flag=True):
    # sim.simulate(I_S=I_S_0, rand_seed=0)#warm up
    t_s = time()
    for i in range(rep_num):
        sim.simulate(I_S=I_S_0, rand_seed=i)
    if print_flag:
        print_run_time('Algorithm 1 sparse', t_s, 'second', rep_num)
...
if __name__ == "__main__":
    ...
    nodes_num_list = [1000, 5000, 10000, 50000, 100000, 500000]
    warm_flag = True
    for n in nodes_num_list:
        ...
        if warm_flag:
            evaluate_performance_of_simulation_model(I_S_0, sim, 1, False)
            warm_flag = False

        evaluate_performance_of_simulation_model(I_S_0, sim, 1)
