...
def evaluate_performance_of_traditional_simulation_model(I_S_0, sim, rep_num, print_flag=True):
    ...
    if print_flag:
        print_run_time('Traditional algorithm', t_s, 'minute', rep_num)
...
if __name__ == "__main__":
    ...
    for n in nodes_num_list:
        ...
        if warm_flag:
            evaluate_performance_of_traditional_simulation_model(I_S_0, sim, 1, False)#warm up
            warm_flag = False
        evaluate_performance_of_traditional_simulation_model(I_S_0, sim, 1)
