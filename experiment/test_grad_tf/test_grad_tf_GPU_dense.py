...
def evaluate_performance_of_gradient_computation_tf_GPU(I_S_0, sim, rep_num, print_flag=True):
    # sim.simulate_and_bp_tf(I_S=I_S_0, GPU_flag=True, dense_flag=True, rand_seed=0)
    t_s = time()
    for i in range(rep_num):
        sim.simulate_and_bp_tf(I_S=I_S_0, GPU_flag=True, dense_flag=True, rand_seed=i)
    if print_flag:
        print_run_time('Gradient Computation Using TensorFlow-GPU-dense', t_s, 'second', rep_num)
