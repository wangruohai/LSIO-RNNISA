...
def evaluate_performance_of_gradient_computation_tf_CPU(I_S_0, sim, rep_num, print_flag=True):
    # sim.simulate_and_bp_tf(I_S=I_S_0, GPU_flag=False, rand_seed=0)#warm up
    t_s = time()
    for i in range(rep_num):
        sim.simulate_and_bp_tf(I_S=I_S_0, GPU_flag=False, rand_seed=i)
    if print_flag:
        print_run_time('Gradient Computation Using TensorFlow-CPU-sparse', t_s, 'second', rep_num)
...
if __name__ == "__main__":
    ...
    nodes_num_list = [1000, 5000, 10000, 50000, 100000, 500000]#[1000, 5000, 10000, 50000, 100000, 500000]#[1000]#
    warm_flag = True
