...
def evaluate_performance_of_simulation_tf_CPU(I_S_0, sim, rep_num, print_flag=True):
    # sim.simulate_tf(I_S=I_S_0, GPU_flag=False, dense_flag=True, rand_seed=0)#warm up
    t_s = time()
    for i in range(rep_num):
        sim.simulate_tf(I_S=I_S_0, GPU_flag=False, dense_flag=True, rand_seed=i)
    if print_flag:
        print_run_time('Simulation Using TensorFlow-CPU-dense', t_s, 'second', rep_num)
