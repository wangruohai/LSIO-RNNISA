...
def evaluate_performance_of_ipa(I_S_0, sim, rep_num, print_flag=True):
    # sim.simulate_and_IPA(I_S=I_S_0, rand_seed=0)#warm up
    t_s = time()
    for i in range(rep_num):
        sim.simulate_and_IPA(I_S=I_S_0, rand_seed=i)
    if print_flag:
        print_run_time('IPA-sparse', t_s, 'minute', rep_num)
...
if __name__ == "__main__":
    ...
    for n in nodes_num_list:
        ...
        if warm_flag:
            evaluate_performance_of_ipa(I_S_0, sim, 1, False)#warm up
            warm_flag = False
